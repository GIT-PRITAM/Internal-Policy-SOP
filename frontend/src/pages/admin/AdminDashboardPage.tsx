      ...
      <div className="space-y-6">
        <section className="rounded-[2rem] border border-white/10 bg-slate-950/80 p-6 shadow-soft backdrop-blur-xl">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.35em] text-indigo-300/70">Admin dashboard</p>
              <h1 className="mt-3 text-4xl font-semibold text-white">Policy governance at a glance</h1>
          ...
          <aside className="rounded-2xl border border-white/10 bg-white/5 p-5 shadow-soft">
            <div className="flex items-center justify-between gap-4">
              <div>
                ...
                <h2 className="text-xl font-semibold text-slate-100">Team compliance pulse</h2>
              </div>
        ...
        <section className="grid gap-4 xl:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)]">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-5 shadow-soft">
            <div className="flex items-center justify-between gap-4">
              <div>
                <h2 className="text-xl font-semibold text-slate-100">Policy activity</h2>
                <p className="text-sm text-slate-400">Recent adoption and engagement trends</p>
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5 shadow-soft">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <h2 className="text-xl font-semibold text-slate-100">Popular policies</h2>
                  <p className="text-sm text-slate-400">Most recently updated policies</p>
                </div>
                <Badge tone="indigo">Top 3</Badge>
              </div>
              <div className="mt-5 space-y-3">
                {topPolicies.length === 0 ? (
                  <div className="rounded-3xl border border-white/10 bg-slate-950/60 p-4 text-slate-300">No policies available yet.</div>
                ) : (
                  topPolicies.map((policy) => (
                    ...
                    >
                      <PolicyCard title={policy.title} badge={policy.status} description={policy.summary ?? 'No summary provided.'} />
                    </button>
                  ))
                )}
              </div>
